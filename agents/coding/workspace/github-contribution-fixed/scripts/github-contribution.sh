...
ISSUE_NUMBER="$3" 
FEATURE_BRANCH="$4"
PROJECTS_ROOT="${5:-${HOME}/IdeaProjects}"  # 第5个参数为根目录，默认为 ~/IdeaProjects

if [ $# -lt 4 ]; then
    echo "用法: $0 <your-username> <owner/repo> <issue-number> <feature-branch-name> [projects-root]"
    echo "示例: $0 linux2010 openclaw/openclaw 31233 fix-auth-vulnerability"
...
echo "🚀 开始 GitHub 贡献流程（修复版 + 自定义根目录）"
echo "用户名: $USERNAME"
echo "项目: $OWNER_REPO"  
